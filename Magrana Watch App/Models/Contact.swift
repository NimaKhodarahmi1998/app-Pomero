...
final class Contact {
    ...
    // Connection (cross-user sync). nil = local-only contact.
    // `connectionZoneOwnerName` is the CKRecordZone.ID owner — `__defaultOwner__`
    // for a zone I created, or the inviter's record name for a zone shared to me.
    var connectionZoneName: String? = nil
    var connectionZoneOwnerName: String? = nil
